...
export WORKDIR=/lus/flare/projects/MatSciAI/xiaoliyan/workdir/alcf-aurora-llm/gpt-oss-120b
...
# NOTE: do NOT use $INCLUDE — modules set it to C include paths. Use GGUF_FILE.
GGUF_FILE=${GGUF_FILE:-gpt-oss-120b-MXFP4.gguf}
...
echo "Downloading $REPO ($GGUF_FILE) → $OUT_DIR"
if command -v hf >/dev/null 2>&1; then
  hf download "$REPO" --include "$GGUF_FILE" --local-dir "$OUT_DIR"
elif command -v huggingface-cli >/dev/null 2>&1; then
  huggingface-cli download "$REPO" --include "$GGUF_FILE" --local-dir "$OUT_DIR"
else
  python -m huggingface_hub.commands.huggingface_cli download "$REPO" \
    --include "$GGUF_FILE" --local-dir "$OUT_DIR"
fi

ls -lh "$OUT_DIR"/*.gguf
# Point smoke/perf default path at HF file (keep local convert as backup if present)
if [ -f "$OUT_DIR/$GGUF_FILE" ]; then
  ln -sfn "$OUT_DIR/$GGUF_FILE" "$OUT_LINK.hf"
  echo "HF GGUF ready: $OUT_DIR/$GGUF_FILE"
